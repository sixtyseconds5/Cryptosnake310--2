...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Count plays today (UTC day)
    const { data: rpcData, error: rpcErr } = await supabase.rpc('rpc_count_scores_today', { p_fid: fid }).catch(()=>({data:null,error:null}))
    let playsToday = 0
    if(rpcData && Array.isArray(rpcData) && rpcData[0] && rpcData[0].count !== undefined) {
      playsToday = Number(rpcData[0].count)
    } else {
      const { count, error } = await supabase.from('scores').select('*', { count: 'exact' }).eq('fid', fid).gte('created_at', new Date().toISOString().slice(0,10)+'T00:00:00Z')
      if(!error) playsToday = Number(count) || 0
    }
